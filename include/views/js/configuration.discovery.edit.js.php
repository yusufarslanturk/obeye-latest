...
<script type="text/javascript">
	...
	function discoveryCheckTypeToString(svcPort) {
		...
		defPorts[ZBX_SVC.telnet] = <?= CJs::encodeJson(_('Telnet')) ?>;
                defPorts[ZBX_SVC.fonetweb] = <?= CJs::encodeJson(_('Fonetweb')) ?>;
		defPorts[ZBX_SVC.agent] = <?= CJs::encodeJson(_('SKYMON agent')) ?>;
				...
				error: function() {
					overlayDialogue({
						...
						'content': jQuery('<span>').text(<?= CJs::encodeJson(
							_('Cannot validate discovery check: invalid request or connection to SKYMON server failed.')
						) ?>),
						'buttons': [
							{
								'title': <?= CJs::encodeJson(_('Cancel')) ?>,
