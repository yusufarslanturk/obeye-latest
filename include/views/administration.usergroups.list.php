...
foreach ($this->data['usergroups'] as $usrgrp) {
	...
	if (isset($usrgrp['users'])) {
		...
		foreach ($userGroupUsers as $user) {
			...
			$users[] = (new CLink(getUserFullname($user), 'skymon.php?action=user.edit&userid='.$user['userid']))
	...
	$userGroupTable->addRow([
		...
		[new CLink(_('Users'), 'skymon.php?action=user.list&filter_usrgrpid='.$userGroupId),
