	...
	$list = (new CList())
		...
		->addItem([
			...
			(new CSpan())
				...
				->addItem(
					new CLink($name, (new CUrl('skymon.php'))
	...
	if ($parent_sysmaps) {
		...
		foreach ($parent_sysmaps as $parent_sysmap) {
			$parent_maps->addItem((new CSpan())->addItem(
				new CLink($parent_sysmap['name'], (new CUrl('skymon.php'))
					->setArgument('action', 'map.view')
...
function getHostAvailabilityTable($host) {
	...
	foreach (['SKY' => '', 'snmp' => 'snmp_', 'jmx' => 'jmx_', 'ipmi' => 'ipmi_'] as $type => $prefix) {
...
function makeAdministrationGeneralMenu($selected) {
	$autoreg_url = (new CUrl('skymon.php'))
