...
$widget = (new CWidget())
	...
	->setControls((new CList([
		(new CForm('get'))
			...
			(new CTag('nav', true,
				(new CList())
					->addItem(new CRedirectButton(_('Create user'), 'skymon.php?action=user.edit'))
				))->setAttribute('aria-label', _('Content controls'))
		]))
	)
	->addItem((new CFilter((new CUrl('skymon.php'))->setArgument('action', 'user.list')))
		->setProfile($data['profileIdx'])
		->setActiveTab($data['active_tab'])
		->addFilterTab(_('Filter'), [
			(new CFormList())->addRow(_('User name'),
...
$table = (new CTableInfo())
	->setHeader([
		...
		make_sorting_header(_('User name'), 'alias', $data['sort'], $data['sortorder']),
...
foreach ($data['users'] as $user) {
	...
	$blocked = ($user['attempt_failed'] >= ZBX_LOGIN_ATTEMPTS)
		? (new CLink(_('Blocked'), 'skymon.php?action=user.unblock&userids[]='.$userid))
	...
	$alias = new CLink($user['alias'], (new CUrl('skymon.php'))
