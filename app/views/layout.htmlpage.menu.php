		...
		(new CList())
			...
			->addItem(CBrandHelper::isRebranded()
				...
				: (new CListItem(
					(new CLink(_('Support'), $data['support_url']))
						...
						->setTitle(_('SKYMON Technical Support'))
				))->addStyle('padding-left:0')
			)
			->addItem(CBrandHelper::isRebranded()
				? null
				: (new CListItem(
					(new CLink('Share', 'https://www.fonetyazilim.com/'))
						->addClass(ZBX_STYLE_TOP_NAV_ZBBSHARE)
						->setAttribute('target', '_blank')
						->setTitle(_('SKYMON Share'))
				))
			)
			...
			->addItem(
				$data['user']['is_guest']
					...
					: (new CLink(null, (new CUrl('skymon.php'))
...
$top_menu = (new CDiv())
	->addItem(
		(new CLink(
			...
			'skymon.php?action=dashboard.view'
		))
