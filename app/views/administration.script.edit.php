...
$scriptFormList = (new CFormList())
	...
	->addRow((new CLabel(_('Execute on'), 'execute_on')),
		(new CRadioButtonList('execute_on', (int) $data['execute_on']))
			->addValue(_('SKYMON agent'), ZBX_SCRIPT_EXECUTE_ON_AGENT)
			->addValue(_('SKYMON server (proxy)'), ZBX_SCRIPT_EXECUTE_ON_PROXY)
			->addValue(_('SKYMON server'), ZBX_SCRIPT_EXECUTE_ON_SERVER)
			->setModern(true)
	)
...
// footer
$cancelButton = (new CRedirectButton(_('Cancel'), 'skymon.php?action=script.list'))->setId('cancel');
...
if ($data['scriptid'] == 0) {
	...
	$deleteButton = (new CRedirectButton(_('Delete'),
		'skymon.php?action=script.delete&sid='.$data['sid'].'&scriptids[]='.$data['scriptid'],
		_('Delete script?')
	))
		->setId('delete');
