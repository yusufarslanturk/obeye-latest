...
$cancelButton = (new CRedirectButton(_('Cancel'), 'skymon.php?action=mediatype.list'))->setId('cancel');
...
if ($data['mediatypeid'] == 0) {
	...
	$deleteButton = (new CRedirectButton(_('Delete'),
		'skymon.php?action=mediatype.delete&sid='.$data['sid'].'&mediatypeids[]='.$data['mediatypeid'],
		_('Delete media type?')
	))
		->setId('delete');
