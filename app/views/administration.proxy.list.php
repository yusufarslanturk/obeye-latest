...
$widget = (new CWidget())
	...
	->setControls((new CTag('nav', true,
		(new CList())
			->addItem(new CRedirectButton(_('Create proxy'), 'skymon.php?action=proxy.edit'))
		))
			->setAttribute('aria-label', _('Content controls'))
	)
	->addItem((new CFilter((new CUrl('skymon.php'))->setArgument('action', 'proxy.list')))
...
foreach ($data['proxies'] as $proxy) {
	...
	$name = new CLink($proxy['host'], 'skymon.php?action=proxy.edit&proxyid='.$proxy['proxyid']);
