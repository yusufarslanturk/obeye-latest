...
if ($data['action'] === 'user.edit') {
	$user_form_list
		->addRow((new CLabel(_('User name'), 'alias'))->setAsteriskMark(),
		...
		$tabs->setFooter(makeFormFooter(
			...
			[
				(new CRedirectButton(_('Delete'),
					'skymon.php?action=user.delete&sid='.$data['sid'].'&userids[]='.$data['userid'],
					_('Delete selected user?')
				))
					->setEnabled(bccomp(CWebUser::$data['userid'], $data['userid']) != 0)
					->setId('delete'),
				(new CRedirectButton(_('Cancel'), 'skymon.php?action=user.list'))->setId('cancel')
			]
		));
	}
	else {
