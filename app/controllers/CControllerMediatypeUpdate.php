...
class CControllerMediatypeUpdate extends CController {
		...
		if (!$ret) {
			switch ($error) {
				case self::VALIDATION_ERROR:
					$response = new CControllerResponseRedirect('skymon.php?action=mediatype.edit');
	...
	protected function doAction() {
		...
		if ($result) {
			$response = new CControllerResponseRedirect('skymon.php?action=mediatype.list&uncheck=1');
